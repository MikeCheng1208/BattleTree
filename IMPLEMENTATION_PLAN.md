...
```js
{
  ...
  groupCount: 1 | 2 | 4 | 8,
  freeSlotCount: 8 | 16 | 24 | 32 | 48 | 64,
  byeSlots: number[],
  refillSlots: number[],
...
主辦手動選對戰格數（`freeSlotCount`：8/16/24/32/48/64，預設依人數選擇可容納的最小選項；人數 > 格數擋下、上限 64 人），名單依配對方式套種子分佈排入並平均分散，剩餘格子為空位。
...
- **24／48 格三強循環**：分別拆成三條 8／16 格淘汰分支，依蛇形種子分佈讓前 3 種子分居不同分支。三名分支勝者進行 3 場循環賽，排名規則為勝場 → 兩人同勝場時直接對戰 → 得失分差 → 總得分 → 編號；三人各 1 勝 1 敗時比分必填。
- 自由模式強制 `groupCount = 1`；8／16／32／64 格維持季軍戰與 mirror 視圖，24／48 格改用三強循環且停用 mirror。榮譽榜與匯出皆支援；有待定格時冠軍不會產生（預期行為）。
...
- 對戰組數對半拆分，左半籤表由左往右匯聚、右半籤表由右往左匯聚，冠亞軍決賽置於正中間。
- 僅在單組且至少 4 人（第一輪 ≥ 2 場）時提供。
...
- 24 格應產生 12 → 6 → 3 場淘汰輪次，再進入 3 場三強循環賽。
- 48 格應產生 24 → 12 → 6 → 3 場淘汰輪次，再進入 3 場三強循環賽。
- 三強循環完成後依勝場與比分產生前三名；三人各 1 勝 1 敗且未輸入比分時不得完成排名。
