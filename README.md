...
BattleTree 是一個純前端單淘汰賽對戰表工具。它支援建立多份對戰表、設定參賽名單、依序或隨機產生賽程、分組賽程、季軍戰、對戰表縮放拖曳、勝者與比分紀錄、抽籤展示、JPG 匯出，以及可替換 Logo / 背景圖的風格設定。
...
- 自由編排：主辦手動選 8／16／32／64 格，名單先排入、左右平均；空格的對戰顯示「待定」不自動晉級，主辦可填入名單剩餘者、第一輪敗者（再戰一次）、手動新增的選手，或將該格確認為輪空。
- 自由編排可在單一待定格中隨機選擇 1 位可用的第一輪敗者，也提供「隨機填入敗者」與「剩餘全部輪空」批次操作；已填的格位可隨時換人或清空，受影響的下游成績會先告知場數並確認後連鎖清除。
- 若有兩位四強敗者，系統會產生第三名爭奪戰。
