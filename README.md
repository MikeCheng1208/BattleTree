...
2. 在設定頁輸入比賽名稱。
3. 設定參賽人數，系統會自動建立名單。
4. 編輯選手姓名，或使用「匯入 CSV」從 Google 表單報名資料一次匯入。
5. 可勾選綠色確認按鈕標記報名成功，或移除不需要的參賽者。
6. 移除參賽者後，原參賽編號會先保留；需要連續編號時再點擊「重新排序參賽編號」。
7. 選擇配對方式：
...
- 空位視為 bye，選手會自動晉級。
- 空對空分支會隱藏，不會形成等待中的比賽。
- 敗部復活模式啟用時，系統會自動偵測原本會連續輪空兩次的支線，第一輪完成後依設定名額把第一輪敗者安插到第一輪後面的空位。
- 敗部復活支援隨機抽選或手動指定，復活者會直接進入主對戰樹一路往上打。
- 對戰表產生後也可透過對戰頁的「啟用敗部復活」按鈕啟用；若受影響的下游場次已有成績，系統會先告知影響場數並要求確認。
- 執行中啟用後，在復活名單安插、相關場次開打前，仍可調整選人方式與名額或停用敗部復活。
...
- `src/composables/useBrackets.js`：對戰表 CRUD、localStorage 儲存、選手更新、分組更新、敗部復活與賽果更新。
